...
class Tree():
    ...
    def __create_tree(self,column_name,data, total_no_of_cols, attribute_used,node):
        if len(column_name)!=0:
            ...
            for i in range (len(data[0]) - 1):
                ent = self.calc(data,i)
                gainDict[i]=ent
            min_col_index=min(gainDict,key=gainDict.get)
            mean=self.find_mean(min_col_index,data)
            node.data = column_name[min_col_index]
            attribute_used += 1
            left_data,right_data=self.split_data(min_col_index,mean,data[1:])
            l,r,lclass,rclass=self.check_thresold(left_data,right_data)
    ...
    def ConfusionMatrix(self,TestData):
        predicted=self.predict(TestData)
        testData = copy.deepcopy(TestData)
        testData.pop(0)
        actual = [data[len(testData[0]) - 1] for data in testData]
